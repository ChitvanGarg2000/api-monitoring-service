...
export class AnalyticService {
    constructor(metricRepository) {
        if (!metricRepository) {
            throw new Error('Metric repository is required');
        }
    ...
    parseTimeFilters = (filters = {}) => {
        ...
        if (!startTime) {
            startTime = new Date()
            startTime.setHours(startTime.getHours() - 24) // default to last 24 hours
        } else {
    ...
    getOverallStats = async (clientId, options = {}) => {
        try {
            const { startTime, endTime } = this.parseTimeFilters(options);
            const stats = await this.metricRepository.getOverallStats(clientId, startTime, endTime);

            const totalHits = parseInt(stats.total_hits || 0);
            const totalErrors = parseInt(stats.error_hits || 0);
            ...
            logger.error('AnalyticService: getOverallStats failed', error);
            throw new AppError('Failed to get overall stats', 500);
        }

    }

    getTopEndpoints = async (clientId, options = {}) => {
        try {
    ...
    getTimeSeries = async (clientId, filters = {}) => {
        try {
