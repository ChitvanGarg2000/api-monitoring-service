...
export class AnalyticController {
    constructor({ analyticService, authService, clientRepository }) {
        if (!analyticService || !authService || !clientRepository) {
            throw new Error('analyticService, authService, and clientRepository are required');
        }
    ...
    ensureCanViewAnalytics = async (req) => {
        if (!req.user?.userId) {
    ...
    resolveTimeRange = (startTime, endTime) => {
        ...
        if ((startTime != null && startTime !== '' && Number.isNaN(start)) ||
            (endTime != null && endTime !== '' && Number.isNaN(end))) {
    ...
    getDashboard = async (req, res, next) => {
        try {
            ...
            const [stats, topEndpoints, recentActivity] = await Promise.all([
                this.analyticService.getOverallStats(finalClientId, timeRange),
                this.analyticService.getTopEndpoints(finalClientId, {
                    limit: 5,
                    startTime: timeRange.startTime,
                    endTime: timeRange.endTime,
                }),
                this.analyticService.getTimeSeries(finalClientId, { ...timeRange, limit: 24 }),
            ]);

            res.status(200).json(
                ResponseFormatter.success(
                    { stats, topEndpoints, recentActivity },
